...
public class UserAwardMapDaoTest {
    ...
    public void testBQueryUserAwardMapList() throws  Exception {
        ...
        PersonInfo custtomer = new PersonInfo();
        //按用户模糊查询
        custtomer.setName("a");
        userAwardMap.setUser(custtomer);
        ...
        assertEquals(2, count);
        //测试queryUserAwardMapById，预期按优先级排列返回第二个奖品信息
        userAwardMap = userAwardMapDao.queryUserAwardMapById(userAwardMapList.get(0).getUserAwardId());
        assertEquals("测试2.0",userAwardMap.getAward().getAwardName());
    }
    ...
    public void testCUpdateUserAwardMap() throws Exception {
        ...
        int effeteNum = userAwardMapDao.updateUserAwardMap(userAwardMapList.get(0));
        assertEquals(1,effeteNum);
    ...
    public void test() {
        ...
        UserAwardMap userAwardMap = new UserAwardMap();
        userAwardMap.setUser(customer);
        List<UserAwardMap> awardMapList = userAwardMapDao.queryUserAwardMapList(userAwardMap, 0, 2);
        System.out.println(awardMapList);
